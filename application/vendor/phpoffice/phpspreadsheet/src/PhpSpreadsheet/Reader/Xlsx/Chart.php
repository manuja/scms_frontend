...
class Chart
{
    ...
    private static function chartDataSeries($chartDetail, $namespacesChartMeta, $plotType)
    {
        ...
        $smoothLine = false;
        $sertestabel = $seriesCategory = $seriesValues = $plotOrder = [];
                    ...
                    foreach ($seriesDetails as $seriesKey => $seriesDetail) {
                        switch ($seriesKey) {
                            ...
                            case 'order':
                                ...
                                break;
                            case 'tx':
                                $sertestabel[$seriesIndex] = self::chartDataSeriesValueSet($seriesDetail, $namespacesChartMeta);
        ...
        return new DataSeries($plotType, $multiSeriesType, $plotOrder, $sertestabel, $seriesCategory, $seriesValues, $smoothLine);
    }
    ...
    private static function chartDataSeriesValuesMultiLevel($seriesValueSet, $dataType = 'n')
    {
        ...
        foreach ($seriesValueSet->lvl as $sertestevelIdx => $sertestevel) {
            foreach ($sertestevel as $seriesValueIdx => $seriesValue) {
                switch ($seriesValueIdx) {
                    case 'ptCount':
                        $pointCount = self::getAttribute($seriesValue, 'val', 'integer');
