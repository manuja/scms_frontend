...
$(document).ready(function(){
    ...
    $(".date-field-clone-end").on("dp.change", function (e) {
     ...
     if($('input[name="al_disable"]').is(':checked')){
       
        $("#al_result_panel").hide();
        $(".type-of-exam").prop('required',false);
        $("input[name='al_exam_year']").prop('required',false);
        $("input[name='user_al_certificate']").prop('required',false);
        $(".al-subject").prop('required',false);
        $(".al-result").prop('required',false);
    }
    
     //disable al result 
    $("#al_disable").click(function(){
//        alert("ok");
        
        if($(this).prop( "checked" )){
            $("#al_result_panel").hide(1000);
            $(".type-of-exam").prop('required',false);
            $("input[name='al_exam_year']").prop('required',false);
            $("input[name='user_al_certificate']").prop('required',false);
            $(".al-subject").prop('required',false);
            ...
            $("#al_result_panel").show(1000);
            $(".type-of-exam").prop('required',true);
            $("input[name='al_exam_year']").prop('required',true);
            $("input[name='user_al_certificate']").prop('required',true);
            $(".al-subject").prop('required',true);
            $(".al-result").prop('required',true);
        }
    });
    ...
    $(".from-date-disable").click(function () {
        ...
        if($(this).prop( "checked" )){
            ...
            parentRow.find(".to_ongoing").removeClass('has-error has-danger');
        }else{
            ...
            parentRowParent.find(".year_of_award").parent().validator('validate');
            parentRow.find(".to_ongoing").addClass('has-error has-danger');
        }
    });
...
//        $("#reg_form").submit();
    ...
    $('#reg_form').submit(function (event) {
        ...
        if ($('#form_edit_mode').val() == '1'){
            var submitPath = '../register/resubmit';
        }else{
            var submitPath = '../register/submit';
        }
        $.ajax({
            type: 'POST',
            url: submitPath,
            ...
            success: function (data) {
                ...
                if(data['db_insert'] == 1){
                    ...
                    if(data['newTempUserEmail']) {
                        // new entry
                        location.replace('../register/success?em=' + data['newTempUserEmail'] + '&ps=' + data['newTempUserAccInfo']['password'] + '&id=' + data['newTempUserAccInfo']['id']);
                    }else{
                        // update
...
//            event.preventDefault();
    ...
    $('.prof_mem_clone_button').click(function (event) {
        ...
        console.log(prof_mem_row_clone_template);
...
//        });
        ...
        prof_mem_row_clone.find(".select-dropdown-tags-clone").select2({
            tags: true,
            createTag: function (params) {
                var term = $.trim(params.term);
    ...
    $(".data_row_clear_button").click(function (event) {
        ...
        this_data_row.find(".select-dropdown-tags").trigger('change');
    ...
    $(".data_row_clear_button_academic").click(function (event) {
        ...
        this_data_row.find(".form-dropdown").prop("selectedIndex", 0);
        this_data_row.find(".select-dropdown-tags, .select-dropdown-tags-clone").trigger('change');
        this_data_row.find(".file_info, .help-block.with-errors").html('');
        this_data_row.find(".has-error.has-danger").removeClass('has-error has-danger');
    });
    ...
    // search by name
    $.get('../MemberRegistration/fetchCorporateMembers/', function(data){
    ...
    $.get('../MemberRegistration/fetchCorporateMembersMemNum/', function(data){
    ...
    inputCorpoMem.change(function(){
        ...
        if( corpo_row_1.find('.corporate-member-mem-name').val() == '' ){
            if(inputCorpoMem.val() == current.name) {
                ...
                var other_proposer = corpo_row_2.find('.corporate-member-mem-name').val();
                if(other_proposer == current.name){ // same propser is in other row
            ...
            if(inputCorpoMem.val() == current.name) {
                ...
                // to prevent same user being picked
                var other_proposer = corpo_row_1.find('.corporate-member-mem-name').val();
                if(other_proposer == current.name){
    ...
    inputCorpoMemNum.change(function(){
        ...
        if( corpo_row_1.find('.corporate-member-mem-name').val() == '' ){
            if(inputCorpoMemNum.val() == current.name) {
                ...
                // to prevent same user being picked
                var other_proposer = corpo_row_2.find('.corporate-member-mem-name').val();
                if(other_proposer == current.name2){
                    $("#proposer_name_w_initials_match_row1").val('');
                }else {
            ...
            if(inputCorpoMemNum.val() == current.name) {
                ...
                // to prevent same user being picked
                var other_proposer = corpo_row_1.find('.corporate-member-mem-name').val();
                if(other_proposer == current.name2){ // same propser is in other row
                    $("#proposer_name_w_initials_match_row2").val('');
                }else {
...
//            $(this).siblings(".old-attachment-preview, .prev-uploaded-file").remove();
    ...
    $("select.type-of-exam").on('change', function (event) {
        ...
        $.ajax({
            type: 'POST',
            url: '../MemberRegistration/getAlSubjects',
            data: {
    ...
    // when user select technical engineer class, add ext attachemtnblock to the form -- start
    $("#user_member_class").on('change', function(event){
        event.preventDefault();
        var block_holder = $("#techeng_exp_holder");
        var block = $("#techeng_exp_block");
        if($(this).val() == '2' || $(this).val() == '3'){ // add exp block
            block_holder.html('');
            var block_clone = block.clone(true);
            block_clone.css('display', 'block').removeAttr("id");
            block_holder.append(block_clone);
        }else{ // remove exp block
            block_holder.html('');
        }
    });
    // when user select technical engineer class, add ext attachemtnblock to the form -- end
});
