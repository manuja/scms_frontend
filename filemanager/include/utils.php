...
if (!function_exists('trans')) {
    ...
    // language
    if (!isset($_SESSION['RF']['language']) || file_exists('lang/' . basename($_SESSION['RF']['language']) . '.php') === false || !is_readable('lang/' . basename($_SESSION['RF']['language']) . '.php')
    ) {
...
{
    if (strpos($path, '../') !== false || strpos($path, './') !== false || strpos($path, '/..') !== false || strpos($path, '..\\') !== false || strpos($path, '\\..') !== false || strpos($path, '.\\') !== false || $path === ".."
    ) {
        return false;
    }
    return true;
 ...
 */
function create_folder($path = null, $path_thumbs = null, $ftp = null, $config = null, $select_type, $divisions, $user_classes)
{
    ...
    if ($ftp) {
        ...
        if ($path && !file_exists($path)) {
            ...
            if ($select_type == 2) {
                $share_with = $divisions;
            } else {
                $share_with = $user_classes;
            }
            require 'connection.php';
            $query = "INSERT INTO " . $db . ".`document_archive` (`folder_name`, `select_type`, `share_with`, `status`) VALUES ('" . $path . "', '" . $select_type . "', '" . $share_with . "', '1');";
            $conn->query($query);
            $conn->close();
