...
$(document).ready(function () {
    ...
    $("#manual_payment_made").click(function (event) {
        ...
        $.ajax({
            url: "../../../MemberTechEngRegistration/manualPaymentMade/",
            data: {
                user_tbl_id: user_id
            },
            dataType: "json",
...
//                console.log(JSON.parse(data));
    ...
    $('#scheduleVivaButton, #viva_results_form_reschedule').on('click', function(event){
        event.preventDefault();
        var applicant_id = $('#applicant_id').val();
        var application_id = $('#application_id').val();
        var redirect_url = $('#redirect_url').val();
        $.ajax({
            url: "../../../ResourcePersonBooking/loadBookingView/2",
            data: {
...
//    jQuery( "#registry_form_2" ).submit(function( event ) {
    ...
    function applicationApprove() {
        ...
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: '../../../MemberTechEngRegistration/afterEditPaymentApplicationApproval',
            data: {
            ...
            success: function (done) {
                ...
                response = JSON.parse(done);
location.reload();
//               window.location.href='../../../applications/techeng/get/'+application_id;
               /* if (response.status === false) {
