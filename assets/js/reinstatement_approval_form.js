...
$(document).ready(function () {
    ...
    $("#manual_payment_made").click(function (event) {
        ...
        $.ajax({
            url: "../../../../MemberReinstatement/manualPaymentMade/",
            data: {
                user_tbl_id: user_id
            },
            dataType: "json",
...
//            fullForm.trigger('submit');
    ...
    $("#form2Submit").click(function (event) {
        ...
        if (fullErr.length <= 0) {
//            fullForm.trigger('submit');
            var sub_amount = parseFloat($('#app_final_amount').val()) + parseFloat($('#subscrip_amount_for_registration').val()) + parseFloat($('#arries_final_amount').val()) + parseFloat($('#surcharge_final_amount').val());
...
//    jQuery( "#registry_form_2" ).submit(function( event ) {
    ...
    function applicationApprove() {
        ...
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: '../../../../MemberReinstatement/afterEditPaymentApplicationApproval',
            data: {
            ...
            success: function (done) {
                ...
                response = JSON.parse(done);
               window.location.href='../../../applications/reinstatement';
               /* if (response.status === false) {
