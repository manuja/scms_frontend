...
class Chart extends WriterPart
{
    ...
    private function writePlotGroup($plotGroup, $groupType, $objWriter, &$catIsMultiLevelSeries, &$valIsMultiLevelSeries, &$plotGroupingType)
    {
        ...
        foreach ($plotSeriesOrder as $plotSeriesIdx => $plotSeriesRef) {
            ...
            //    Labels
            $plotSertestabel = $plotGroup->getPlotLabelByIndex($plotSeriesRef);
            if ($plotSertestabel && ($plotSertestabel->getPointCount() > 0)) {
                $objWriter->startElement('c:tx');
                $objWriter->startElement('c:strRef');
                $this->writePlotSertestabel($plotSertestabel, $objWriter);
                $objWriter->endElement();
                $objWriter->endElement();
            }
     ...
     *
     * @param DataSeriesValues $plotSertestabel
     * @param XMLWriter $objWriter XML Writer
     */
    private function writePlotSertestabel($plotSertestabel, $objWriter)
    {
        if ($plotSertestabel === null) {
            return;
        }

        $objWriter->startElement('c:f');
        $objWriter->writeRawData($plotSertestabel->getDataSource());
        $objWriter->endElement();

        $objWriter->startElement('c:strCache');
        $objWriter->startElement('c:ptCount');
        $objWriter->writeAttribute('val', $plotSertestabel->getPointCount());
        $objWriter->endElement();

        foreach ($plotSertestabel->getDataValues() as $plotLabelKey => $plotLabelValue) {
