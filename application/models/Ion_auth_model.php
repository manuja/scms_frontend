...
class Ion_auth_model extends CI_Model
{
    ...
    {
        // var_dump($password, $email, $additional_data);die();
        ...
        $profile_data = array(
            'user_id' => $id,
            'name_w_initials' => $additional_data['name_initials']
        );
        //create profile
        $profile =  array_merge($this->_filter_data($this->tables['user_profile'], $additional_data), $profile_data);

        // var_dump($profile);die();


        $this->db->insert($this->tables['user_profile'], $profile);
    ...
    public function register_member($array1,$array2,$array3 = array())
    {
     

        $email = $array1['username'];
        $password = $array1['nic'];
        ...
        if ($this->identity_check($email)) {
            // print("in->if");
        $this->set_error('account_creation_duplicate_identity');
           $result = array('status' => 504, 'message' => 'Already Created Account ! Please Check Your Email ');
            return $result;
        }
        //print("in->else");
        // die();
        ...
        $user_data = array_merge($this->_filter_data($this->tables['users'], $userdata), $data);
        ...
        $user_profile = array_merge($this->_filter_data($this->tables['user_profile'], $userprfile), $data2);


        $this->db->insert($this->tables['user_profile'], $user_profile);
        ...
        if( $array3 != "" ){
         ...
         $cer_length = count($array3);

        for ($x = 0; $x < $cer_length; $x++) {


            $sq2 = "insert into member_certificates  (user_id,document_name,created_by) values (?,?,?)";
            $this->db->query($sq2, array($id,$array3[$x]->postname,$array2['created_by']));
        }
        ...
        if(isset($id)){
            $result = array('status' => 200, 'message' => 'Data Saved successfully', 'user_id' => $id);
          return $result;
        }else{
            $result = array('status' => 500, 'message' => 'Somthing Went Wrong');
          return $result;
        } 
    ...
    public function login($identity, $password, $remember = FALSE)
    {
        ...
        $query = $this->db->select($this->identity_column . ', id, password, active, last_login,first_name,last_name')
            ->where($this->identity_column, $identity)
            ->limit(1)
            ->order_by('id', 'desc')
            ->get($this->tables['users']);
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                if ($user->active == 0 || $user->active == -3 || $user->active == 2 || $user->active == 3 || $user->active == 5 || $user->active == 6 || $user->active == 7) {
                    $this->trigger_events('post_login_unsuccessful');
    ...
    public function users($groups = NULL)
    {
        ...
        $this->db->where_in('active',array(1,0,7,4,5));
    ...
    public function users_edit($groups = NULL)
    {
        $this->trigger_events('users');
        $this->db->where('is_admin!=', 1);
        $this->db->where_in('active',array(1,0,7,4,5,2,3));
        if (isset($this->_ion_select) && !empty($this->_ion_select)) {
            foreach ($this->_ion_select as $select) {
                $this->db->select($select);
            }
    ...
    public function users_notify($groups = NULL)
    {
        $this->trigger_events('users');
        $this->db->where('is_admin!=', 1);
        $this->db->where_in('active',array(1,4));
        if (isset($this->_ion_select) && !empty($this->_ion_select)) {
            foreach ($this->_ion_select as $select) {
                $this->db->select($select);
            }
    ...
    public function members($groups = NULL)
    {
        $this->trigger_events('users');
        $this->db->where('is_admin!=', 1);
        $this->db->where('active!=', -1);
        $this->db->where('in_source!=', 1);
        if (isset($this->_ion_select) && !empty($this->_ion_select)) {
            foreach ($this->_ion_select as $select) {
                $this->db->select($select);
            }
        ...
        foreach ($group_ids as $group_id) {
            ...
            if ($this->db->insert(
                ...
                if (isset($this->_cache_groups[$group_id])) {
                    ...
                    if ($group_name == '' || $group_name == null || isEmpty($group_name)) {
                        $group_name = 0;
                    } else {
    ...
    public function get_sub_groups($group_id)
    {
        ...
        if ($group_id != NULL || $group_id != 0) {
            ...
            $query = $this->db->query("SELECT * FROM `groups` WHERE parent_id= " . $group_id);
            return $query->result();
        } else {
            return array();
        }
     ...
     * @author test
     */
    public function create_sub_group($data)
    {
    ...
    public function update($id, array $data)
    {
        ...
        $data['name_w_initials'] = $data['name_initials'];

        $profile_data = $this->_filter_data($this->tables['user_profile'], $data);
        $this->db->update($this->tables['user_profile'], $profile_data, array('user_id' => $user->id));
    ...
    public function updatemember($id, array $data1, $data2)
    {
        $this->trigger_events('pre_update_user');
        ...
        if (array_key_exists($this->identity_column, $data1) && $this->identity_check($data1[$this->identity_column]) && $user->{$this->identity_column} !== $data1[$this->identity_column]) {
            $this->db->trans_rollback();
            $this->set_error('account_creation_duplicate_identity');
        ...
        // Filter the data passed
        $data = $this->_filter_data($this->tables['users'], $data1);
        ...
        $profile_data = $this->_filter_data($this->tables['user_profile'], $data2);
        $this->db->update($this->tables['user_profile'], $profile_data, array('user_id' => $user->id));
    ...
    public function set_session($user)
    {
        ...
        $session_data = array(
            ...
            'email' =>  $user->{$this->identity_column},
    ...
    public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
    {
        ...
        if ($additional_data) {
            $additional_data = array('description' => $additional_data['desc']);
        }
    ...
    public function change_password_firstime_login($identity, $new)
    {

        $this->load->model(array('User_model', 'Side_bar_m'));
        ...
        if (isset($form_data['password'])) {
            ...
            if ($successfully_changed_password_in_db) {
                ...
                $this->User_model->newPasswordSet($user_id);
                $this->trigger_events(array('post_change_password', 'post_change_password_successful'));
                $this->set_message('password_change_successful');
                ...
                if ($this->ion_auth->is_admin()) {
                    ...
                    //Redirect user to his/her relevant page auth/load_dashboard()
    ...
    public function password_verification($id, $password, $use_sha1_override = FALSE)
    {
        if (empty($id) || empty($password)) {
            return FALSE;
        }
    ...
    public function mobile_app_login($identity, $password, $remember = FALSE)
    {
        $this->trigger_events('pre_login');
        ...
        if (empty($identity) || empty($password)) {
            $this->set_error('login_unsuccessful');
            return array('status' => 401, 'message' => 'Username or Password is Empty.');
        }
        ...
        if ($this->is_max_login_attempts_exceeded($identity)) {
            ...
            return array('status' => 401, 'message' => 'Maximum login attempts exceeded');
        }
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                ...
                $this->db->select('COUNT(*) AS usercountval');
                $this->db->from('user_profile');
                $this->db->where('user_id', $user->id);
                $this->db->where('status', 1);
                $user_tbl_idstate = $this->db->get()->row()->usercountval;



                if ($user->active == 0 || $user->active == -1 || $user_tbl_idstate == 0) {
                    $this->trigger_events('post_login_unsuccessful');
                    $this->set_error('login_unsuccessful_not_active');

                    return array('status' => 415, 'message' => 'Account not Activated.');
                }
                ...
                //$tokenappend=$mob_password.$user->id;
                $token = $user->id . uniqid();
                $token = hash('sha256', $token);
                //$token = crypt(substr(md5(rand(1000000,100000000)), 0, 7),$tokenappend);
                //$token =preg_replace('/[^a-zA-Z0-9\']/',"456",$token);
                $expired_at = date("Y-m-d H:i:s", strtotime('+12 hours'));
                $this->db->trans_start();

                $this->db->select('COUNT(*) AS usercount');
                $this->db->from('mobile_users_authentication');
                $this->db->where('users_id', $user->id);
                $usercount = $this->db->get()->row()->usercount;
                if ($usercount > 0) {

                    $this->db->where('users_id', $user->id)->update('mobile_users_authentication', array('token' => $token, 'expired_at' => $expired_at));
                } else {
                    $this->db->insert('mobile_users_authentication', array('users_id' => $user->id, 'token' => $token, 'expired_at' => $expired_at));
                }
                ...
                if ($this->db->trans_status() === FALSE) {
                    ...
                    $this->trigger_events(array('post_login', 'post_login_successful'));
                    $this->set_message('login_successful');
                    return array("status" => 200, "user_id" => $user->id, 'token' => $token, 'message' => 'Login Successful.');
                }
            }
        } else {

            return array('status' => 401, 'message' => 'Username or Password is Incorrect.');
        }
    }

    public function web_login($identity, $password, $remember = FALSE)
    {
        $this->trigger_events('pre_login');
        ...
        if ($query->num_rows() === 1) {
            ...
            $query1 = $this->db->select('password, salt')
                ->where('id', $user->id)
                ->limit(1)
                ->order_by('id', 'desc')
                ->get($this->tables['users']);

            $hash_password_db = $query1->row();

            if ($web_password == $hash_password_db) {
                ...
                $this->db->select('COUNT(*) AS usercount');
                $this->db->from('web_users_authentication');
                $this->db->where('users_id', $user->id);
                $usercount = $this->db->get()->row()->usercount;
                if ($usercount > 0) {

                    $this->db->where('users_id', $user->id)->update('web_users_authentication', array('token' => $token, 'expired_at' => $expired_at));
                } else {
                    $this->db->insert('web_users_authentication', array('users_id' => $user->id, 'token' => $token, 'expired_at' => $expired_at));
                }
    ...
    public function getuserbygroup($data){
        ...
        $sq2 = "select user_id from users_groups where group_id = ? and status = 1 ";
    ...
    public function get_dashboard_user_count()
    {
        ...
        $user_deleted = 0;
            ...
            $this->db->where('active', 1);
            $user_active = $this->db->get()->num_rows();


            $this->db->select('*');
            $this->db->from('users');
            $this->db->where('is_admin', 0);
            $this->db->where_in('active', array(0,1,2,3,4,5));
            $user_all = $this->db->get()->num_rows();
            ...
            $user_inactive = $this->db->get()->num_rows();


            $this->db->select('*');
            $this->db->from('users');
            $this->db->where('is_admin', 0);
            $this->db->where('active', 6);
            $user_deleted = $this->db->get()->num_rows();


            $results = array('all' => $user_all, 'active' => $user_active, 'inactive' => $user_inactive , 'deleted' => $user_deleted);
